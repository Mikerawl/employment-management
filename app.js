...
const db = mysqul.createCondection({
  ...
  port:3003
  password: "",
  database: "employee_manangement_db"
});
...
const viewEmployeeByManager = (manager) => {
  ...
  SELECT
      ...
      employee.las_name AS 'last Name',
...
const addEmployee = () => {
  ...
  inquirer
      .prompt([
          ...
          name: 'first_name',
          type: 'input',
          message: 'Employees\'s first name'
      },
      {
          name: 'Last_name',
          type: 'input',
          message: 'Employees\'s last name'
      },
      {
          name: 'role',
          type: 'rawlist',
...

  {
      ...
      VALUES(?, ?, ? ,?)
      `,[first_name, last_name, role_id, manager_id],function(err, res) {
          if(err) throw err;
          console.log('added employee successed') ;J
      mainOption();
      }); 
  }).catch(err => err);   
}  
        ...
        db.query(`SELECT id, title AS 'name' FROM  role`, function(err, res) {
              if(err) throw err;
                  inquirer
                    ...
                    console.log(chosen);
                    const roleId = find(Id)(chosen.role, res);
                    console.log(roleId);
                        db.query(`
                        UPDATE employee
                        SET role_id = ?
...
const updateEmployeeManager = () => {
      db.query(selectIdandNameFromEmployee, function(err, res) {
          if(err) throw err;
              ...
              .then((chosen) => {
              //console log(chosen);
              const employee_id = findId(chosen.employee, res);
              ...
              db.query(selectIdandNameFromEmployee, function(err, res) {
                ...
                // console.log(chosen);
                const managerId = find(Id)(chosen.manager, res);
